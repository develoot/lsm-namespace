...
static char lsm_buff[BUFF_SIZE];
...
static ssize_t lsmns_read(struct file *fp, char __user *user_buff, size_t count,
			  loff_t *position)
{
	ssize_t size;
	read_lock(&buff_lock);
	size = simple_read_from_buffer(user_buff, count, position, lsm_buff,
				       BUFF_SIZE);
	read_unlock(&buff_lock);
	return size;
}
			   ...
			   size_t count, loff_t *position)
{
	...
	memset(lsm_buff, 0, BUFF_SIZE);
	size = simple_write_to_buffer(lsm_buff, BUFF_SIZE, position, user_buff,
				      count);
